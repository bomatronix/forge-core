...
3. Each affected workspace: builds its apps, uploads one or more function artifacts to S3 (for example `api/crud-${sha}.zip`, `api/presign-${sha}.zip`, `api/authorizer-${sha}.zip`), updates the TFE workspace variable with artifact metadata (`version`, `prefix`, `functions`), triggers a Terraform run
...
When one built app bundle is reused by multiple deployed Lambda functions, add an `artifacts` map:
