...
const entry = existsSync(lambdaEntry)
  ...
  : existsSync(handlerEntry)
    ...
    : (() => {
        throw new Error(
          `No entry point found for app: ${app}. Expected ${lambdaEntry} or ${handlerEntry}`,
        );
      })();
...
const external = [
  ...
  ...(app === 'migrate'
    ? []
    : [
        // AWS SDK v3 is available in the managed Lambda runtime for the API bundles.
        '@aws-sdk/*',
      ]),
];
...
if (app === 'migrate') {
  const migrationsOut = `dist/lambda/${app}/drizzle/migrations`;
  rmSync(migrationsOut, { force: true, recursive: true });
  cpSync('drizzle/migrations', migrationsOut, { recursive: true });
  console.log(`Migrations copied:     ${migrationsOut}`);
}
